...
Additionally, jobs can be loaded from disk. Note that since `JSONL` saving is lossy, and does not include fields such as the `status` and `error_detail`, jobs can only be loaded from 

**`MessageJob`:**

A `MessageJob` should be used to send a single message to the model, optionally along with a system prompt. The model will respond to the message, and the result stored in memory.